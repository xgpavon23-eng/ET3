...
function setLang(lang=''){
   ...
   for(var clave in traduccion) {
    ...
    try {traduccion_dinamica_entidad(document.getElementById("search_entidad").value);}
    catch{}
}
...
function traduccion_dinamica_entidad(nombreentidad)
{
    var traduccion = eval('textos_'+nombreentidad+'[getCookie("lang")]')
    ...
    for(var clave in traduccion)
    {
        ...
        for (var elem in elementos)
        {
            try {elementos[elem].innerHTML = traduccion[clave];}
            catch {}
        }
